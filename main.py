...
# Ignore SQLAlchemy-related warnings from Pandas
...
def connect_db():
    ...
    host = 'dbpostsuadara.postgres.database.azure.com'
    database = 'phone_db'
...
# Deletion of a specific row by selecting from available IDs
with col1.expander('Delete Data Entry', expanded=False):
    st.header('Delete a Sales Record')
    
    # Query the database for available IDs
    cur.execute("SELECT id FROM phone_sales ORDER BY id;")
    result = cur.fetchall()
    ids = [i[0] for i in result]  # Extract the IDs from query result
    
    # Selectbox for available record IDs
    record_id_to_delete = st.selectbox('Select the ID of the record to delete', ids)
    
    # Delete button
    if st.button('Delete Record'):
        # SQL query to delete a record
        delete_query = f"DELETE FROM phone_sales WHERE id = {record_id_to_delete};"
        try:
            cur.execute(delete_query)
            conn.commit()
            st.success(f'Record with ID {record_id_to_delete} has been successfully deleted.')
